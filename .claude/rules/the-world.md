# The world — time, touch, structure, the chain, the grammar
...
## The chain
31. SETTLED — Blind chains. A depth is a chain of rooms joined by doors; at
    each room, 2–3 doors, each sensed in one line. No map UI exists — the
    road ahead is only ever the doors in front of you.
...
34. SETTLED — Knowledge attaches to things, not places. Clues, names, and
    ...
    survive both death and the reseed.
...
36. DEFAULT — A run is seeded at waking. The whole arrangement derives from
    one seed: senses are stable, resume is exact, and rerolling the
    labyrinth costs what it should — a death.
...
38. SETTLED — Rules, not templates. The generator obeys constraints
    (adjacency bans, per-depth guarantees, key-in-first-half /
    lock-in-last-half bands, fight-count bands), each tested against
    art. 33. Blind play cannot dodge cruelty, so mercy lives in the math.
