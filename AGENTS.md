...
6. Complete the Asana task with a one-line comment: what landed, how
   ...
   links.
7. Name the task in the PR title or body (`Task: <ID>`) so a reader can
   find the spec. Scope is kept by the agent and the reviewer, not by
   CI — staying inside SCOPE (rule 3) is still binding.
